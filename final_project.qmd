...
# Calculate quantiles
quantiles <- quantile(filled_with_avg$yield,probs = c(0.25,0.50,0.75))

# Separate into groups
yield1 <- filled_with_avg |>
  filter(yield <= quantiles['25%']) |>
  select(yield)

yield2 <- filled_with_avg |>
  filter(yield > quantiles['25%'] & yield <= quantiles['50%']) |>
  select(yield)

yield3 <- filled_with_avg |>
  filter(yield > quantiles['50%'] & yield <= quantiles['75%']) |>
  select(yield)

yield4 <- filled_with_avg |>
  filter(yield > quantiles['75%']) |>
  select(yield)

# mean and variance of each group in a data frame
yield_groups_var_and_mean <- data.frame(
  group = c("yield1","yield2","yield3","yield4"),
  
  mean = c(mean(yield1$yield),
           mean(yield2$yield),mean(yield3$yield),
           mean(yield4$yield)),
  
  variance = c(var(yield1$yield),
               var(yield2$yield),var(yield3$yield),
               var(yield4$yield))
)
```
...
```{r}
fit_poi <- glm(variance ~ mean, data= yield_groups_var_and_mean, family = poisson(link='log'))
summary(fit_poi)
...
```{r}
# Quasi poisson model
fit_qpoi <- glm(variance ~ mean, data=yield_groups_var_and_mean, family = quasipoisson(link = 'log'))
summary(fit_qpoi)

cat("Quasi Poisson Deviance:",fit_qpoi$deviance)
cat("Quasi Poisson Residuals:",fit_qpoi$residuals)
cat("Quasi Poisson R^2:", 1 - fit_qpoi$deviance/fit_qpoi$null.deviance)
```
```{r}
# Negative binomial model
fit_nb <- MASS::glm.nb(variance ~ mean, data= yield_groups_var_and_mean, link='log')
summary(fit_nb)

cat("NegBin Deviance:",fit_nb$deviance)
cat("NegBin Residuals:",fit_nb$residuals)
cat("NegBin R^2:", 1 - fit_nb$deviance/fit_nb$null.deviance)
```

```{r}
nb_pred <- predict(fit_nb, newdata = yield_groups_var_and_mean, type = 'response')
qpoi_pred <- predict(fit_qpoi, newdata = yield_groups_var_and_mean, type = 'response')

yield_groups_var_and_mean$nb_pred <- nb_pred
yield_groups_var_and_mean$qpoi_pred <- qpoi_pred

yield_groups_var_and_mean |>
  ...
  geom_text(x=50000,y=6e+09,label = paste("NB R^2: ", round(1 - fit_nb$deviance/fit_nb$null.deviance, 2))) + 
  geom_text(x=50000, y=5.5e+09, label = paste("QPoi R^2: ", round(1 - fit_qpoi$deviance/fit_qpoi$null.deviance, 3)))
```
The plot we created compares the real values vs the predicted ones of each model.By observing the plot we can clearly tell that the quasi poisson model is better at predicting & explaining our data than the negative binomial model.
