...
yield_by_cntry <- filled_with_avg |>
  na.omit() |>
...
yield_mean_var <- 
  data.frame(
    cntry = unique(yield_by_cntry$Country),
    mean = unique(yield_by_cntry$mean),
    variance = unique(yield_by_cntry$variance)
  )
...
```{r}
fit_poi <- glm(variance ~ mean, data= yield_mean_var, family = poisson(link='log'))
summary(fit_poi)
...
```{r}
# Quasi poisson model
fit_qpoi <- glm(variance ~ mean, data=yield_mean_var, family = quasipoisson(link = 'log'))
summary(fit_qpoi)
...
fit_nb <- MASS::glm.nb(variance ~ mean, data= yield_mean_var, link='log', control = glm.control(maxit=1e7))
summary(fit_nb)
...
```{r}
nb_pred <- predict(fit_nb, newdata = yield_mean_var, type = 'response')
qpoi_pred <- predict(fit_qpoi, newdata = yield_mean_var, type = 'response')

yield_mean_var$nb_pred <- nb_pred
yield_mean_var$qpoi_pred <- qpoi_pred
...
yield_mean_var |>
  ...
  geom_text(x=50000,y=4e+10,label = paste("NB R^2: ", round(1 - fit_nb$deviance/fit_nb$null.deviance, 2))) + 
  geom_text(x=50000, y=4.5e+10, label = paste("QPoi R^2: ", round(1 - fit_qpoi$deviance/fit_qpoi$null.deviance, 3)))
```
